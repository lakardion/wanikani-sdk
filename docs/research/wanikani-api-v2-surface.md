...
*Doc/example discrepancies worth knowing:* the attribute table omits `level`, but the single-assignment and start-assignment example payloads include `level` (int). The create-review `resources_updated.assignment` example additionally shows `passed` (Bool) and `resurrected` (Bool) fields. Prerequisites for `unlocked_at`: components reached SRS stage 5 once, and user level ≥ subject level.
...
`context_sentences` (obj[]), `meaning_mnemonic` (string), `parts_of_speech` (string[]), `pronunciation_audios` (obj[]). No `readings`, no `component_subject_ids` (it *is* kana). Same nested object shapes as vocabulary.
...
- **Reviews and resets:** never change once recorded (but reviews are created frequently) → safe for long-term storage. *(Caveat: reviews are now deprecated/unpersisted server-side.)*
...
- **Only breaking changes trigger a new revision.** A breaking change = anything changing the *existing structure* of a response (e.g. renaming a field).
